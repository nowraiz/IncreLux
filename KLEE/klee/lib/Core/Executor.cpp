...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      if (bi->getSuccessor(0)->size() > 0) {
          /// add the first instruction that would have been executed
          /// if this path was feasible
	...
	branches.first->encode.addpath("true : " + rso.str());
        ...
        pathSerializer.addTrainingTuple(ins, true, valid);
      } else {
          // the branch is infeasible
          pathSerializer.addTrainingTuple(ins, false, valid);
      }

      /// the original list of instructions
	...
	branches.second->encode.addpath("false : " + rso.str());
        // the branch is feasible
        pathSerializer.addTrainingTuple(ins2, true, valid);
      } else {
          pathSerializer.addTrainingTuple(ins2, false, valid);
      }
      int result;
  ...
  case Instruction::IndirectBr: {
    ...
    // the targets that end up being infeasible
